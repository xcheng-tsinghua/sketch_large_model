...
def main(args):
    ...
    model = getattr(models, args.model)(args=args)  # args.model = ULIP_PN_SSG
...
def test_zeroshot_3d_core(test_loader, model, tokenizer, args=None):
    ...
    with torch.no_grad():
        ...
        for l in labels:
            ...
            class_embeddings = class_embeddings / class_embeddings.norm(dim=-1, keepdim=True)  # [1280]
        ...
        for i, (pc, target, target_name) in enumerate(test_loader):
            ...
            # encode pc
            pc_features = utils.get_model(model).encode_pc(pc)  # [4,1280]
            ...
            for idx, name in enumerate(target_name):
                # print(f"-------- top1_accurate.item(): {top1_accurate.item()}")
                # print(f"-------- type(top1_accurate.item()): {type(top1_accurate.item())}")
